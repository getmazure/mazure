...
class MazureException(Exception):
    pass
...
class CertificateCreator:
    ...
    def validate(self) -> None:
        ...
        if not os.path.isfile(CertificateCreator.cakey):
            raise MazureException(f"Cannot find {CertificateCreator.cakey}")
        if not os.path.isfile(CertificateCreator.cacert):
            raise MazureException(f"Cannot find {CertificateCreator.cacert}")
        if not os.path.isfile(CertificateCreator.certkey):
            raise MazureException(f"Cannot find {CertificateCreator.certkey}")
        if not os.path.isdir(CertificateCreator.certdir):
            raise MazureException(f"Cannot find {CertificateCreator.certdir}")
        ...
        try:
            ...
            with open(test_file_location, "w", encoding="utf-8") as file:
                file.write("test")
    ...
    def create(self, path: str) -> str:
        ...
        with CertificateCreator.lock:
            ...
            if not os.path.isfile(certpath):
                ...
                template = f"{self.certdir.rstrip('/')}/req.conf.tmpl"
                ...
                with open(config_template_name, "w", encoding="utf-8") as f:
